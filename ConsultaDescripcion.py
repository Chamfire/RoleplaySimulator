...
class ConsultaDescripcion:
    ...
    def consultaDescripcion(self):
        # Inicializa el cliente (usa tu API Key)
        # --- Tu lógica de Razas y Clases (se mantiene igual) ---
        raza = ""
        piel = ""
        consideracion = ""
        genero_text = ""
        ...
        if self.personaje.tipo_raza == "Elfo":
            raza = "elfo"
            piel = "de piel verde"
            consideracion = "En mi mundo, los elfos se consideran viejos de 450 a 750 años. Traduce tweed por tejido y evita anglicismos."
            genero_text = "Es un elfo..." if self.personaje.genero == "hombre" else "Es una elfa..."
        elif self.personaje.tipo_raza == "Enano":
            raza = "enano"
            piel = "omite referencias al color de piel"
            consideracion = "Los enanos viven hasta 350 años. Traduce tweed por tejido y evita anglicismos."
            genero_text = "Es un enano..." if self.personaje.genero == "hombre" else "Es una enana..."

        # --- Configuración del Prompt ---
        # Cohere usa 'preamble' para el rol del sistema
        preamble = "Eres un dungeon master de Dnd 5e y me estás ayudando a hacerme la ficha de personaje."
        
        prompt_usuario = f"""Describe la apariencia física de un {raza} que pesa {self.personaje.peso} libras, 
        tiene {self.personaje.edad} años de edad, y {piel} en únicamente un párrafo. 
        {consideracion} Limítate a ser creativo y divertido, y comienza directamente tu respuesta con la frase "{genero_text}" """
        ...
        # --- Limpieza de texto ---
        if "." in self.response_good:
            self.response_good = self.response_good.rsplit(".", 1)[0] + "."
        self.response_good = self.response_good.lstrip()

        print(f"Descripción generada: {self.response_good}")
        
        # Hilo para cambiar de pantalla
        hiloCambiaScreen = threading.Thread(target=self.cambiarScreenThread)
        hiloCambiaScreen.start()
